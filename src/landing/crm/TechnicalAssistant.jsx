...
import {
  ALL_DEPTS,
  fetchTechnicalAssistantIndex,
  fetchTechnicalAssistantList,
  selectTechnicalAssistantIndexRows,
  selectTechnicalAssistantIndexStatus,
  selectTechnicalAssistantIndexError,
  selectTechnicalAssistantListItems,
  selectTechnicalAssistantListStatus,
  selectTechnicalAssistantListError,
  selectTechnicalAssistantDeptName,
} from "../../redux/crm/technicalAssistantSlice";
...
const fmxStyles = `
  ...
  .fmx-avatar{
    ...
    background: radial-gradient(140px 140px at 30% 20%, rgba(201,75,80,.16), transparent 60%), linear-gradient(180deg, rgba(0,0,0,.03), rgba(0,0,0,.06));
    position: relative;
    display:grid;
    place-items:center;
  }
...
export function TechnicalAssistantContent() {
  const dispatch = useDispatch();
  const navigate = useNavigate();
  const location = useLocation();

  const indexRows = useSelector(selectTechnicalAssistantIndexRows);
  const indexStatus = useSelector(selectTechnicalAssistantIndexStatus);
  const indexError = useSelector(selectTechnicalAssistantIndexError);

  const listItems = useSelector(selectTechnicalAssistantListItems);
  const listStatus = useSelector(selectTechnicalAssistantListStatus);
  const listError = useSelector(selectTechnicalAssistantListError);
  const deptNameFromStore = useSelector(selectTechnicalAssistantDeptName);
  ...
  useEffect(() => {
    ensureFontAwesome();
    dispatch(fetchTechnicalAssistantIndex());
  }, [dispatch]);
  ...
  useEffect(() => {
    ...
    dispatch(fetchTechnicalAssistantList({ deptUuid: selectedDeptUuid }));
    setPage(1);
  ...
  const subtitle =
    selectedDeptUuid === ALL_DEPTS
      ? "Showing technical assistants from all departments."
      : `Technical assistants of ${deptNameFromStore || "selected department"}`;
        ...
        <div className="fmx-tools">
          <div className="fmx-search">
            ...
            <input
              ...
              type="search"
              placeholder="Search technical assistant (name/designation/qualification)…"
            />
          </div>
        ...
        <>
          <div className="fmx-grid">
            {currentItems.map((item, idx) => {
              ...
              const name =
                pick(item, ["name", "user_name", "title"]) ||
                "Technical Assistant";
...
export default TechnicalAssistantContent;