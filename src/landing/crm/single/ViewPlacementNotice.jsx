...
/* Redux */
import {
  fetchPlacementNoticeView,
  clearPlacementNoticeView,
  selectPlacementNoticeView,
  resolveUrl,
  parseAttachments,
  formatDate,
} from "../../../redux/crm/single/placementNoticeViewSlice";
...
export default function ViewPlacementNotice() {
  const dispatch = useDispatch();
  const { slug } = useParams();

  const { notice, loading, error } = useSelector(selectPlacementNoticeView);
  const [copied, setCopied] = useState(false);

  useEffect(() => {
    if (slug) dispatch(fetchPlacementNoticeView(slug));
    return () => dispatch(clearPlacementNoticeView());
  }, [dispatch, slug]);

  const date = formatDate(
    notice?.publish_at || notice?.created_at || notice?.updated_at
  );

  const attachments = useMemo(
    () => parseAttachments(notice?.metadata || notice?.metadata_json),
    [notice?.metadata, notice?.metadata_json]
  );
  ...
  const getMetaText = () => {
    ...
    const dept =
      Array.isArray(notice?.departments)
        ? notice.departments
            .map((d) => d?.name || d?.title || d?.department_name)
            .filter(Boolean)
            .join(", ")
        : (notice?.department_name || notice?.department || notice?.departmentTitle || "");

    if (dept) items.push({ key: "dept", label: dept });

    const recruiter =
      notice?.recruiter?.name ||
      notice?.recruiter?.title ||
      notice?.recruiter_name ||
      notice?.recruiterName ||
      "";

    if (recruiter) items.push({ key: "recruiter", label: recruiter });

    const role = notice?.role_title || notice?.role || "";
    if (role) items.push({ key: "role", label: role });

    const ctc = notice?.ctc != null && String(notice.ctc).trim() !== "" ? String(notice.ctc) : "";
    if (ctc) items.push({ key: "ctc", label: `CTC: ${ctc}` });

    const deadline = formatDate(notice?.last_date_to_apply);
    if (deadline) items.push({ key: "deadline", label: `Last date: ${deadline}` });

    if (notice?.is_featured_home === 1 || notice?.is_featured_home === true || String(notice?.is_featured_home) === "1") {
      items.push({ key: "featured", label: "Featured" });
    }

    return items;
  };

  const metaItems = getMetaText();
    ...
    <>
      ...
      <main className="announcement-container">
        <div className="announcement-header">
          ...
          {metaItems.length > 0 && (
            <div className="announcement-meta" style={{ display: "flex", flexWrap: "wrap", gap: "10px", marginTop: "14px" }}>
              {metaItems.map((item) => (
          ...
          <div className="announcement-actions">
            ...
            {notice?.apply_url && (
              <a
                ...
                href={resolveUrl(notice.apply_url)}
                target="_blank"
        ...
        {notice?.banner_image_url && (
          <LazySection minHeight={280}>
            <figure className="announcement-cover">
              <img
                src={resolveUrl(notice.banner_image_url)}
                alt="Banner image"
                loading="lazy"
              />
            </figure>
