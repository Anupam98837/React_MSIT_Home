...
/* Redux */
import {
  fetchCareerNoticeView,
  clearCareerNoticeView,
  selectCareerNoticeView,
  resolveUrl,
  parseAttachments,
} from "../../../redux/crm/single/careerNoticeViewSlice";
...
export default function ViewCareerNotice() {
  const dispatch = useDispatch();
  const { slug } = useParams();

  const { notice, loading, error } = useSelector(selectCareerNoticeView);
  const [copied, setCopied] = useState(false);

  useEffect(() => {
    if (slug) dispatch(fetchCareerNoticeView(slug));
    return () => dispatch(clearCareerNoticeView());
  }, [dispatch, slug]);

  const attachments = useMemo(
    () => parseAttachments(notice?.attachments_json || notice?.attachments || notice?.files),
    [notice?.attachments_json, notice?.attachments, notice?.files]
  );
    ...
    try {
      if (navigator.share) {
        await navigator.share({
          title: notice?.title || document.title,
  ...
  const buildBodyHTML = (item) => {
    ...
    else if (item?.content) html += item.content;
    else if (item?.description) html += item.description;
    else if (item?.details) html += item.details;
