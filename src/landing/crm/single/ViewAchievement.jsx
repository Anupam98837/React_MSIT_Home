...
export default function ViewAchievement() {
  ...
  const { achievement, loading, error } = useSelector(selectAchievementView);
  ...
  const attachments = useMemo(
    ...
    [achievement?.attachments_json]
  );

  const departmentName =
    achievement?.department?.name ||
    achievement?.department?.title ||
    achievement?.department_name ||
    achievement?.department_title ||
    "";
    ...
    try {
      if (navigator.share) {
        await navigator.share({
          title: achievement?.title || document.title,
  ...
  const buildBodyHTML = (a) => {
    let html = "";

    if (a?.body) html += a.body;
  ...
  const getImage = () => {
    ...
    const candidates = [
      achievement.cover_image_url,
      achievement.cover_image,
      achievement.image,
      achievement.banner_image,
      achievement.cover,
      achievement.notice_image,
      achievement.hero_image,
      achievement.thumbnail,
      achievement?.banner?.url,
      achievement?.image?.url,
      achievement?.media?.[0]?.url,
        ...
        <div className="announcement-header">
          ...
          {departmentName ? (
            <div className="notice-meta">
              ...
              {achievement?.is_featured_home === 1 ||
              achievement?.is_featured_home === true ||
              String(achievement?.is_featured_home) === "1" ? (
          ...
          <LazySection eager minHeight={0}>
            <div
              ...
              dangerouslySetInnerHTML={{
                __html: buildBodyHTML(achievement),
              }}
            />
          </LazySection>
        ...
        {coverImageUrl ? (
          <LazySection minHeight={280}>
            <section className="announcement-cover">
              <img
                src={coverImageUrl}
                alt={achievement?.title || "Banner image"}
                loading="lazy"
                onError={(event) => {
