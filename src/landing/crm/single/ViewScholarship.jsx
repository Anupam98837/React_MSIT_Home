...
/* Redux */
import {
  fetchScholarshipView,
  clearScholarshipView,
  selectScholarshipView,
  resolveUrl,
  parseAttachments,
  formatDate,
} from "../../../redux/crm/single/scholarshipViewSlice";
...
export default function ViewScholarship() {
  ...
  const { scholarship, loading, error } = useSelector(selectScholarshipView);
  const [copied, setCopied] = useState(false);

  useEffect(() => {
    if (slug) dispatch(fetchScholarshipView(slug));
    return () => dispatch(clearScholarshipView());
  }, [dispatch, slug]);

  const publishDate = formatDate(
    scholarship?.publish_at ||
      scholarship?.created_at ||
      scholarship?.updated_at
  );

  const attachments = useMemo(
    () => parseAttachments(scholarship?.attachments || scholarship?.attachments_json),
    [scholarship?.attachments, scholarship?.attachments_json]
  );

  const departmentName =
    scholarship?.department?.name ||
    scholarship?.department?.title ||
    ...
    try {
      if (navigator.share) {
        await navigator.share({
          title: scholarship?.title || document.title,
  ...
  const buildBodyHTML = (n) => {
    ...
    else if (n?.content) html += n.content;
    else if (n?.description) html += n.description;
  ...
  const getImage = () => {
    ...
    const candidates = [
      ...
      scholarship.image,
      scholarship.banner_image,
      scholarship.cover,
      scholarship.hero_image,
      scholarship.thumbnail,
      scholarship?.banner?.url,
      scholarship?.image?.url,
      scholarship?.media?.[0]?.url,
      scholarship?.images?.[0]?.url,
    ];

    return candidates.map((i) => resolveUrl(i)).find(Boolean) || "";
  };
        ...
        <div className="announcement-header">
          ...
          {departmentName ? (
            <div className="notice-meta">
              ...
              <span className="meta-pill meta-pill-date" id="metaDate">
          ...
          <LazySection eager minHeight={0}>
            <div
              ...
              dangerouslySetInnerHTML={{
                __html: buildBodyHTML(scholarship),
              }}
            />
          </LazySection>
        ...
        {coverImageUrl ? (
          <LazySection minHeight={280}>
            <section className="announcement-cover">
              <img
                src={coverImageUrl}
                alt={scholarship?.title || "Banner image"}
                loading="lazy"
                onError={(e) => {
