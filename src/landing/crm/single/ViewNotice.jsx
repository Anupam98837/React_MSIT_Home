...
import {
  ...
  resolveUrl,
  parseAttachments,
} from "../../../redux/crm/single/noticeViewSlice";
...
export default function ViewNotice() {
  ...
  const { notice, loading, error } = useSelector(selectNoticeView);
  const [copied, setCopied] = useState(false);
 
  useEffect(() => {
    if (slug) dispatch(fetchNoticeView(slug));
 
    return () => dispatch(clearNoticeView());
  }, [dispatch, slug]);
 
  const attachments = useMemo(
    () => parseAttachments(notice?.attachments || notice?.attachments_json),
...
<span>{departmentName}</span>
              ...
              {notice?.is_featured_home === 1 ||
              notice?.is_featured_home === true ||
              String(notice?.is_featured_home) === "1" ? (
