...
import {
  ...
  fetchFacultyIndex,
  fetchFacultyList,
  selectFacultyIndexRows,
  selectFacultyIndexStatus,
  selectFacultyIndexError,
  selectFacultyListItems,
  selectFacultyListStatus,
  selectFacultyListError,
  selectFacultyDeptName,
} from "../../redux/crm/facultySlice";
...
export function FacultyContent() {
  ...
  const indexRows = useSelector(selectFacultyIndexRows);
  const indexStatus = useSelector(selectFacultyIndexStatus);
  const indexError = useSelector(selectFacultyIndexError);

  const listItems = useSelector(selectFacultyListItems);
  const listStatus = useSelector(selectFacultyListStatus);
  const listError = useSelector(selectFacultyListError);
  const deptNameFromStore = useSelector(selectFacultyDeptName);
  ...
  const departments = useMemo(() => {
    return (indexRows || [])
      ...
      .map((row) => ({
        ...
        count: parseInt(row?.order?.faculty_count ?? 0, 10) || 0,
      }))
  ...
  useEffect(() => {
    ...
    dispatch(fetchFacultyList({ deptUuid: selectedDeptUuid }));
    setPage(1);
  ...
  const subtitle =
    selectedDeptUuid === ALL_DEPTS
      ? "Showing faculty members from all departments."
      : `Faculty members of ${deptNameFromStore || "selected department"}`;
  ...
  const filteredItems = useMemo(() => {
    ...
    return (listItems || []).filter((item) => {
      const name = clean(pick(item, ["name", "user_name"])).toLowerCase();
      const desig = clean(
        ...
        <div className="fmx-tools">
          <div className="fmx-search">
            ...
            <input
              ...
              type="search"
              placeholder="Search faculty (name/designation/qualification)…"
            />
          </div>
        ...
        <>
          <div className="fmx-grid">
            {currentItems.map((item, idx) => {
              const slugPath = buildProfilePath(item);
              const key =
                clean(pick(item, ["user_uuid", "uuid", "__profile_slug"])) ||
                `${clean(pick(item, ["name", "user_name"]))}-${idx}`;

              const name = clean(pick(item, ["name", "user_name"])) || "Faculty Member";
              const desig =
                clean(
                  pick(item, ["designation"]) ||
                    decodeMaybeJson(item?.metadata)?.designation ||
              ...
              const img = normalizeUrl(
                pick(item, ["image_full_url", "image"])
              );
              ...
              return (
                <article
                  ...
                  onClick={() => cardNavigate(slugPath)}
                  onKeyDown={(e) => {
                    if (e.key === "Enter" || e.key === " ") {
                      e.preventDefault();
                      cardNavigate(slugPath);
                    }
                  }}
                >
